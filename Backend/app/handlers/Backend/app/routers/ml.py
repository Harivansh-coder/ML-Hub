...
# from app.handlers.summary import generate_abstractive_summary
...
def summarize_text(text: MLRequest):
    ...
    return {"summary": "generate_abstractive_summary(text.text)"}
...
def check_similarity(playload: SimilarityRequest):
    ...
    similarity_scaore = 55
    if similarity_scaore > 0.6:
        return {"message": "similar content", "similarity_score": similarity_scaore}
    else:
        return {"message": "not similar content", "similarity_score": similarity_scaore}
